/*
  ...
  Made a quick fix to use it as CommonJs module
    by Jorrit Duin <jorrit@venalis.net>
 
  
  ChangeLog:
   - 04.10.2009: Ininitial port at http://devhouseberlin.de/
   
   - 18.09.2010: Fork of http://github.com/janl/mustache.js
...
var Mustache = {
  ...
  render: function(template, view) {
    Mustache.stack = Mustache.stack + " ";
    // fail fast
    if(template.indexOf("{{") == -1) {
      return template;
    }
    Mustache.context = context = Mustache.merge((Mustache.context || {}), view);
    var html = Mustache.render_section(template);
    // restore context, recursion might have messed it up
    Mustache.context = context;
    return Mustache.render_tags(html);
  },
  ...
  render_partial: function(name) {
    ...
    switch(typeof evil_name) {
      case "string":
        return Mustache.to_html(evil_name, "");
      case "object":
        var tpl = name + "_template";
        return Mustache.to_html(eval(tpl), evil_name);
      default:
  ...
  find: function(name) {
    name = Mustache.trim(name);
    // print(Mustache.stack + "find(" + name + ")");
